...
export default function Stage() {
  ...
  const lookAtPoint = useMemo(() => new THREE.Vector3(...portalPosition), [])
  ...
  useFrame(({ camera }) => {
    ...
    if (camera.position.z < portalPosition[2] + portalRadius) {
      mouseX = 0.0
      mouseY = 0.5
  ...
  return (
    <>
      ...
      <mesh ref={portal} position={[0, -0.6, -10]}>
        <cylinderGeometry args={[portalRadius, 2, 1, 64]} />
